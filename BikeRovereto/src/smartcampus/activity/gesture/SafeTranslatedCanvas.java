...
/**
 * An implementation of {@link ISafeCanvas} that wraps a {@link Canvas} and
 * adjusts drawing calls to the wrapped Canvas so that they are relative to an
 * origin that is always at the center of the screen.<br />
 * <br />
...
public class SafeTranslatedCanvas extends Canvas implements ISafeCanvas
{
	...
	@Override
	public boolean clipRect(double left, double top, double right,
			double bottom, Op op)
	{
		return getWrappedCanvas().clipRect((float) (left + xOffset),
				(float) (top + yOffset), (float) (right + xOffset),
				(float) (bottom + yOffset), op);
	}
	...
	{
		return getWrappedCanvas().clipRect(left + xOffset, top + yOffset,
				right + xOffset, bottom + yOffset);
	}
	...
	@Override
	public void drawArc(Rect oval, float startAngle, float sweepAngle,
			boolean useCenter, SafePaint paint)
	{
		getWrappedCanvas().drawArc(this.toOffsetRectF(oval, sRectF),
				startAngle, sweepAngle, useCenter, paint);
	}

	@Override
	public void drawBitmap(Bitmap bitmap, double left, double top,
			SafePaint paint)
	{
		getWrappedCanvas().drawBitmap(bitmap, (float) (left + xOffset),
				(float) (top + yOffset), paint);
	}
	...
	@Override
	public void drawBitmap(int[] colors, int offset, int stride, double x,
			double y, int width, int height, boolean hasAlpha, SafePaint paint)
	{
		getWrappedCanvas().drawBitmap(colors, offset, stride,
				(float) (x + xOffset), (float) (y + yOffset), width, height,
				hasAlpha, paint);
	}
	...
	{
		getWrappedCanvas().drawBitmap(colors, offset, stride, x + offset,
				y + offset, width, height, hasAlpha, paint);
	}

	@Override
	public void drawBitmapMesh(Bitmap bitmap, int meshWidth, int meshHeight,
			double[] verts, int vertOffset, int[] colors, int colorOffset,
			SafePaint paint)
	{
		getWrappedCanvas().drawBitmapMesh(bitmap, meshWidth, meshHeight,
				this.toOffsetFloatAry(verts, sFloatAry), vertOffset, colors,
				colorOffset, paint);
	}
	...
	{
		getWrappedCanvas().drawCircle((float) (cx + xOffset),
				(float) (cy + yOffset), radius, paint);
	}
			...
			double stopY, SafePaint paint)
	{
		...
		stopX += xOffset;
		stopY += yOffset;
		getWrappedCanvas().drawLine((float) startX, (float) startY,
	...
	{
		getWrappedCanvas().drawLines(this.toOffsetFloatAry(pts, sFloatAry),
				offset, count, paint);
	}
	...
	{
		getWrappedCanvas().drawPoints(this.toOffsetFloatAry(pts, sFloatAry),
				offset, count, paint);
	}
	...
	public void drawPosText(char[] text, int index, int count, double[] pos,
			SafePaint paint)
	{
		getWrappedCanvas().drawPosText(text, index, count,
				this.toOffsetFloatAry(pos, sFloatAry), paint);
	}

	@Override
	public void drawPosText(String text, double[] pos, SafePaint paint)
	{
		getWrappedCanvas().drawPosText(text,
				this.toOffsetFloatAry(pos, sFloatAry), paint);
	}
			...
			SafePaint paint)
	{
		...
		top += yOffset;
		bottom += yOffset;
		getWrappedCanvas().drawRect((float) left, (float) top, (float) right,
				(float) bottom, paint);
	}
	...
	{
		getWrappedCanvas().drawRoundRect(this.toOffsetRectF(rect, sRectF), rx,
				ry, paint);
	}
	...
	{
		getWrappedCanvas().drawText(text, (float) (x + xOffset),
	...
	@Override
	public void drawText(CharSequence text, int start, int end, double x,
			double y, SafePaint paint)
	{
		getWrappedCanvas().drawText(text, start, end, (float) (x + xOffset),
				(float) (y + yOffset), paint);
	}
	...
	public void drawTextOnPath(char[] text, int index, int count,
			...
			SafePaint paint)
	{
		getWrappedCanvas().drawTextOnPath(text, index, count, path, hOffset,
				vOffset, paint);
	}

	@Override
	public void drawTextOnPath(String text, SafeTranslatedPath path,
	...
	@Override
	public void drawVertices(VertexMode mode, int vertexCount, double[] verts,
			int vertOffset, float[] texs, int texOffset, int[] colors,
			int colorOffset, short[] indices, int indexOffset, int indexCount,
			SafePaint paint)
	{
		getWrappedCanvas().drawVertices(mode, vertexCount,
				this.toOffsetFloatAry(verts, sFloatAry), vertOffset, texs,
	...
	@Override
	public boolean quickReject(double left, double top, double right,
			double bottom, EdgeType type)
	{
		...
		top += yOffset;
		bottom += yOffset;
		return getWrappedCanvas().quickReject((float) left, (float) top,
				(float) right, (float) bottom, type);
	}
	...
	public boolean quickReject(Rect rect, EdgeType type)
	{
		...
		return getWrappedCanvas().quickReject(this.toOffsetRectF(rect, sRectF),
				type);
	}
	...
	public void rotate(float degrees, double px, double py)
	{
		...
		// getWrappedCanvas().rotate(degrees, (float) (px + xOffset), (float)
		// (py + yOffset));
		getWrappedCanvas().translate(-this.xOffset, -this.yOffset);
	}
	...
	@Override
	public int saveLayer(double left, double top, double right, double bottom,
			SafePaint paint, int saveFlags)
	{
		return getWrappedCanvas().saveLayer((float) (left + xOffset),
				(float) (top + yOffset), (float) (right + xOffset),
				(float) (bottom + yOffset), paint, saveFlags);
	}

	@Override
	public int saveLayer(Rect bounds, SafePaint paint, int saveFlags)
	{
		int result = getWrappedCanvas().saveLayer(
				this.toOffsetRectF(bounds, sRectF), paint, saveFlags);
		return result;
	}

	@Override
	public int saveLayerAlpha(double left, double top, double right,
			double bottom, int alpha, int saveFlags)
	{
		return getWrappedCanvas().saveLayerAlpha((float) (left + xOffset),
				(float) (top + yOffset), (float) (right + xOffset),
				(float) (bottom + yOffset), alpha, saveFlags);
	}

	@Override
	public int saveLayerAlpha(Rect bounds, int alpha, int saveFlags)
	{
		return getWrappedCanvas().saveLayerAlpha(
				this.toOffsetRectF(bounds, sRectF), alpha, saveFlags);
	}
	...
	{
		getWrappedCanvas().scale(sx, sy, (float) (px + xOffset),
				(float) (py + yOffset));
	}
	...
	/**
	 * Helper function to convert a Rect to RectF and adjust the values of the
	 * Rect by the offsets.
	 */
	protected final RectF toOffsetRectF(Rect rect, RectF reuse)
	{
		...
		reuse.set(rect.left + xOffset, rect.top + yOffset,
