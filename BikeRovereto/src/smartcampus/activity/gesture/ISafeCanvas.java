...
/**
 * The ISafeCanvas interface is designed to work Android's issues with large
 * canvases.<br />
 * <br />
 * The internal representation of canvas coordinates in the Skia graphics
 * library is float. Canvas sizes are specified as integers. At high zoom
 * levels, the canvas sizes get to the high end of the integer data type which
 * subsequently get rounded off when represented as floats in Skia. This causes
 * drawing anomalies such as jagged edges or distorted shapes that progressively
 * get worse as the zoom level increases. The issue becomes visibly noticeable
 * around zoom level 18 which is commonly available amongst most online map tile
 * providers.<br />
 * <br />
 * To prevent this issue we can't pass large values to the native Android
 * methods. To accomplish that, we must intercept all values being passed to the
 * canvas draw methods and translate them to a coordinate system where the
 * origin (0,0) is the center of the screen. We then draw them to a canvas that
 * has the same coordinate system. We also prevent passing coordinate parameter
 * values as floats, instead accepting doubles.<br />
 * 
 * @see {@link SafeTranslatedCanvas}, {@link SafeTranslatedPath},
 *      {@link SafePaint}
 * 
 * @author Marc Kurtz
 * 
...
public interface ISafeCanvas
{
	 ...
	 * {@link UnsafeCanvasHandler}.
	 */
	public void getUnsafeCanvas(UnsafeCanvasHandler handler);

	/**
	 * Gets the wrapped canvas. This canvas will have a coordinate system where
	 * the origin is at the center of the screen, but will not automatically
	 * adjust values passed to its drawing methods.
	 */
	public Canvas getWrappedCanvas();

	/**
	 * Gets this safe canvas as an Android {@link Native} class. This canvas
	 * will have a coordinate system where the origin is at the center of the
	 * screen, and will automatically adjust values passed to its drawing
	 * methods by {@link #getXOffset()} and {@link #getYOffset()}.
	 */
	...
	/**
	 * <p>
	 * Returns the target density of the canvas. The default density is derived
	 * from the density of its backing bitmap, or {@link Bitmap#DENSITY_NONE} if
	 * there is not one.
	 * </p>
	 * 
	 ...
	 * target density of the canvas itself, as well as the density of its
	 * backing bitmap via {@link Bitmap#setDensity(int) Bitmap.setDensity(int)}.
	 * 
	 * @param density
	 *            The new target density of the canvas, which is used to
	 ...
	 * to translate,scale,rotate,skew,concat or clipRect,clipPath will all
	 * operate as usual, but when the balancing call to restore() is made, those
	 * calls will be forgotten, and the settings that existed before the save()
	 * will be reinstated.
	 * 
	 ...
	 * clipRect,clipPath will all operate as usual, but when the balancing call
	 * to restore() is made, those calls will be forgotten, and the settings
	 * that existed before the save() will be reinstated.
	 * 
	 * @param saveFlags
	 *            flag bits that specify which parts of the Canvas state to
	 *            save/restore
	 * @return The value to pass to restoreToCount() to balance this save()
	 */
	 ...
	 * offscreen bitmap. All drawing calls are directed there, and only when the
	 * balancing call to restore() is made is that offscreen transfered to the
	 * canvas (or the previous layer). Subsequent calls to translate, scale,
	 * rotate, skew, concat or clipRect, clipPath all operate on this copy. When
	 * the balancing call to restore() is made, this copy is deleted and the
	 * previous matrix/clip state is restored.
	 * 
	 * @param bounds
	 *            May be null. The maximum size the offscreen bitmap needs to be
	 *            (in local coordinates)
	 * @param paint
	 *            This is copied, and is applied to the offscreen when restore()
	 *            is called.
	 * @param saveFlags
	 ...
	 */
	public abstract int saveLayer(double left, double top, double right,
			double bottom, SafePaint paint, int saveFlags);
	 ...
	 *            coordinates)
	 * @param alpha
	 *            The alpha to apply to the offscreen when when it is drawn
	 *            during restore()
	 * @param saveFlags
	 *            see _SAVE_FLAG constants
	 ...
	 */
	public abstract int saveLayerAlpha(double left, double top, double right,
			double bottom, int alpha, int saveFlags);
	 ...
	 * 
	 * Example: int count = canvas.save(); ... // more calls potentially to
	 * save() canvas.restoreToCount(count); // now the canvas is back in the
	 * same state it was before the initial // call to save().
	 * 
	 * @param saveCount
	 ...
	 * 
	 * @param matrix
	 *            The matrix to replace the current matrix with. If it is null,
	 ...
	 * 
	 * @param left
	 *            The left side of the rectangle to intersect with the current
	 *            clip
	 * @param top
	 *            The top of the rectangle to intersect with the current clip
	 * @param right
	 *            The right side of the rectangle to intersect with the current
	 *            clip
	 * @param bottom
	 ...
	 * @return true if the resulting clip is non-empty
	 */
	public abstract boolean clipRect(double left, double top, double right,
			double bottom, Region.Op op);
	 ...
	 * matrix, clipRegion() assumes its argument is already in the coordinate
	 * system of the current layer's bitmap, and so not transformation is
	 * performed.
	 * 
	 * @param region
	 ...
	 * matrix, would lie completely outside of the current clip. Call this to
	 * check if an area you intend to draw into is clipped out (and therefore
	 * you can skip making the draw calls). Note: for speed it may return false
	 * even if the path itself might not intersect the clip (i.e. the bounds of
	 * the path intersects, but the path does not).
	 * 
	 * @param path
	 *            The path to compare with the current clip
	 * @param type
	 *            true if the path should be considered antialiased, since that
	 *            means it may affect a larger area (more pixels) than
	 *            non-antialiased.
	 ...
	 * 
	 * @param left
	 *            The left side of the rectangle to compare with the current
	 *            clip
	 * @param top
	 *            The top of the rectangle to compare with the current clip
	 * @param right
	 *            The right side of the rectangle to compare with the current
	 *            clip
	 * @param bottom
	 *            The bottom of the rectangle to compare with the current clip
	 * @param type
	 *            true if the rect should be considered antialiased, since that
	 *            means it may affect a larger area (more pixels) than
	 *            non-antialiased.
	 * @return true if the rect (transformed by the canvas' matrix) does not
	 *         intersect with the canvas' clip
	 */
	public abstract boolean quickReject(double left, double top, double right,
			double bottom, EdgeType type);
	 ...
	 * @param count
	 *            The number of values to process, after skipping offset of
	 ...
	 */
	public abstract void drawPoints(double[] pts, int offset, int count,
			SafePaint paint);
	 ...
	 */
	public abstract void drawLine(double startX, double startY, double stopX,
			double stopY, SafePaint paint);

	/**
	 * Draw a series of lines. Each line is taken from 4 consecutive values in
	 * the pts array. Thus to draw 1 line, the array must contain at least 4
	 * values. This is logically the same as drawing the array as follows:
	 * drawLine(pts[0], pts[1], pts[2], pts[3]) followed by drawLine(pts[4],
	 * pts[5], pts[6], pts[7]) and so on.
	 * 
	 * @param pts
	 *            Array of points to draw [x0 y0 x1 y1 x2 y2 ...]
	 * @param offset
	 ...
	 * @param count
	 *            The number of values in the array to process, after skipping
	 *            "offset" of them. Since each line uses 4 values, the number of
	 *            "lines" that are drawn is really (count >> 2).
	 * @param paint
	 *            The paint used to draw the points
	 */
	public abstract void drawLines(double[] pts, int offset, int count,
			SafePaint paint);
	 ...
	 */
	public abstract void drawRect(double left, double top, double right,
			double bottom, SafePaint paint);
	 ...
	 * then nothing will be drawn. The circle will be filled or framed based on
	 ...
	 */
	public abstract void drawCircle(double cx, double cy, float radius,
			SafePaint paint);
	 ...
	 * start angle modulo 360.
	 * </p>
	 * 
	 * <p>
	 * If the sweep angle is >= 360, then the oval is drawn completely. Note
	 * that this differs slightly from SkPath::arcTo, which treats the sweep
	 * angle modulo 360. If the sweep angle is negative, the sweep angle is
	 ...
	 * 
	 * @param oval
	 *            The bounds of oval used to define the shape and size of the
	 *            arc
	 * @param startAngle
	 ...
	 * @param useCenter
	 *            If true, include the center of the oval in the arc, and close
	 *            it if it is being stroked. This will draw a wedge
	 * @param paint
	 *            The paint used to draw the arc
	 */
	public abstract void drawArc(Rect oval, float startAngle, float sweepAngle,
			boolean useCenter, SafePaint paint);
	 ...
	 */
	public abstract void drawRoundRect(Rect rect, float rx, float ry,
			SafePaint paint);
	 ...
	 * 
	 * <p>
	 * If the bitmap and canvas have different densities, this function will
	 * take care of automatically scaling the bitmap to draw at the same density
	 * as the canvas.
	 * 
	 * @param bitmap
	 ...
	 */
	public abstract void drawBitmap(Bitmap bitmap, double left, double top,
			SafePaint paint);
	 ...
	 * 
	 * <p>
	 * This function <em>ignores the density associated with the bitmap</em>.
	 ...
	 * @param dst
	 *            The rectangle that the bitmap will be scaled/translated to fit
	 *            into
	 * @param paint
	 *            May be null. The paint used to draw the bitmap
	 */
	public abstract void drawBitmap(Bitmap bitmap, Rect src, Rect dst,
			SafePaint paint);
	 ...
	 *            May be null. The paint used to draw the bitmap
	 */
	public abstract void drawBitmap(int[] colors, int offset, int stride,
			double x, double y, int width, int height, boolean hasAlpha,
			SafePaint paint);
	 ...
	 *            May be null. The paint used to draw the bitmap
	 */
	public abstract void drawBitmap(Bitmap bitmap, Matrix matrix,
			SafePaint paint);
	 ...
	 * @param meshWidth
	 *            The number of columns in the mesh. Nothing is drawn if this is
	 *            0
	 * @param meshHeight
	 *            The number of rows in the mesh. Nothing is drawn if this is 0
	 * @param verts
	 *            Array of x,y pairs, specifying where the mesh should be drawn.
	 *            There must be at least (meshWidth+1) * (meshHeight+1) * 2 +
	 *            meshOffset values in the array
	 * @param vertOffset
	 *            Number of verts elements to skip before drawing
	 * @param colors
	 *            May be null. Specifies a color at each vertex, which is
	 *            interpolated across the cell, and whose values are multiplied
	 *            by the corresponding bitmap colors. If not null, there must be
	 *            at least (meshWidth+1) * (meshHeight+1) + colorOffset values
	 *            in the array.
	 * @param colorOffset
	 ...
	 *            May be null. The paint used to draw the bitmap
	 */
	public abstract void drawBitmapMesh(Bitmap bitmap, int meshWidth,
			int meshHeight, double[] verts, int vertOffset, int[] colors,
			int colorOffset, SafePaint paint);
	 ...
	 * @param vertexCount
	 *            The number of values in the vertices array (and corresponding
	 *            texs and colors arrays if non-null). Each logical vertex is
	 *            two values (x, y), vertexCount must be a multiple of 2.
	 * @param verts
	 ...
	 * @param colors
	 *            May be null. If not null, specifies a color for each vertex,
	 *            to be interpolated across the triangle.
	 * @param colorOffset
	 *            Number of values in colors to skip before drawing.
	 * @param indices
	 *            If not null, array of indices to reference into the vertex
	 *            (texs, colors) array.
	 * @param indexCount
	 ...
	 */
	public abstract void drawVertices(VertexMode mode, int vertexCount,
			double[] verts, int vertOffset, float[] texs, int texOffset,
			int[] colors, int colorOffset, short[] indices, int indexOffset,
			int indexCount, SafePaint paint);
	 ...
	 */
	public abstract void drawText(char[] text, int index, int count, double x,
			double y, SafePaint paint);
	 ...
	 *            The paint used for the text (e.g. color, size, style)
	 */
	public abstract void drawText(String text, int start, int end, double x,
			double y, SafePaint paint);

	/**
	 * Draw the specified range of text, specified by start/end, with its origin
	 * at (x,y), in the specified Paint. The origin is interpreted based on the
	...
	public abstract void drawText(CharSequence text, int start, int end,
			double x, double y, SafePaint paint);
	 ...
	 *            The paint used for the text (e.g. color, size, style)
	 */
	public abstract void drawPosText(char[] text, int index, int count,
			double[] pos, SafePaint paint);
	 ...
	 * specified path. The paint's Align setting determins where along the path
	 * to start the text.
	 * 
	 * @param text
	 *            The text to be drawn
	 ...
	 * @param hOffset
	 *            The distance along the path to add to the text's starting
	 *            position
	 * @param vOffset
	 *            The distance above(-) or below(+) the path to position the
	 *            text
	 * @param paint
	 *            The paint used for the text (e.g. color, size, style)
	 */
	public abstract void drawTextOnPath(char[] text, int index, int count,
			SafeTranslatedPath path, float hOffset, float vOffset,
			SafePaint paint);
	 ...
	 *            The paint used for the text (e.g. color, size, style)
	 */
	public abstract void drawTextOnPath(String text, SafeTranslatedPath path,
