...
public class GetReportsTask extends AsyncTask<String, Void, ArrayList<Report>> {
	...
	protected ArrayList<Report> doInBackground(String... data) {
		HttpGet httpg;
		try {
			String url = Tools.SERVICE_URL + data[0] + "/" + Tools.CITY_CODE
					+ "/" + URLEncoder.encode(data[1],"UTF-8") + "/" + Tools.REPORTS_REQUEST;
			httpg = new HttpGet(url);
			...
			for (int i = 0; i < reportsArrayJSON.length(); i++) {
				...
				Report.Type reportType = Report.Type.stringToType(reportJSON
				...
				Report report = new Report(reportType, details, objectType,
						objectId, date);
			...
			Log.e(this.getClass().getCanonicalName(), e1.toString());
